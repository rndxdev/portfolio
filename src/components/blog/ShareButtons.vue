<template>
  <div class="border-t border-border pt-8 mt-12" role="group" aria-label="Share this post">
    <p class="text-sm text-muted mb-4">Share this post</p>
    <div class="flex flex-wrap gap-3">
      <a
        ...
        class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-surface border border-border text-sm text-muted hover:text-text hover:border-accent/50 transition-all duration-200"
      >
        <svg aria-hidden="true" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" v-html="platform.icon" />
      ...
      >
        <svg aria-hidden="true" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path v-if="!copied" stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
...
const platforms = computed(() => [
  {
    name: 'X',
    url: `https://x.com/intent/tweet?text=${encodedTitle.value}&url=${encodedUrl.value}`,
  ...
  {
    name: 'Email',
    url: `mailto:?subject=${encodedTitle.value}&body=${encodedTitle.value}%0A%0A${encodedUrl.value}`,
